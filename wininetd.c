...
static LPVOID winet_prepare_env(portmap_t *pm, SOCKET asock,
                                struct sockaddr_in *saddr);
unsigned int __stdcall winet_thread_proc(void *data);
static int winet_handle_client(portmap_t *pm, SOCKET asock,
                               struct sockaddr_in *saddr);
...
static char *timestamp(void) {
  time_t now;
  static char buf[sizeof "2000-01-01T00:00:00Z"];
...
static void __pWin32Error(int level, DWORD eNum, const char *fmt,
                          va_list args) {
  char emsg[1024];
...
static int pump_s2p(SOCKET sRead, HANDLE hWrite) {
...
static int pump_p2s(HANDLE hRead, SOCKET sWrite) {
  char buf[2048], *p, *pend;
  int nw;
  DWORD nr;
...
static DWORD WINAPI thr_p2s(LPVOID lpThreadParameter) {
  int rc;
  ...
  winet_log(WINET_LOG_MESSAGE, "[%s] %s: thread (%d) exit\n", WINET_APPNAME,
            timestamp(), tid);
...
static int winet_create_listeners(void) {
  int i, reuse, timeo, nodelay;
  struct sockaddr_in saddr;
  struct linger ling;

  for (i = 0; i < npmaps; i++) {
    if ((pmaps[i].sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) ==
        INVALID_SOCKET) {
    ...
    reuse = 1;
    if (setsockopt(pmaps[i].sock, SOL_SOCKET, SO_REUSEADDR, (char *)&reuse,
                   sizeof(reuse))) {
      winet_log(WINET_LOG_ERROR,
                "[%s] unable to set socket option: opt=SO_REUSEADDR\n",
                WINET_APPNAME);
      return -1;
    }
    ...
    if (sk_timeout > 0) {
      ...
      if (setsockopt(pmaps[i].sock, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeo,
                     sizeof(timeo))) {
        winet_log(WINET_LOG_ERROR,
                  "[%s] unable to set socket option: opt=SO_RCVTIMEO\n",
                  WINET_APPNAME);
        return -1;
      }
      timeo = sk_timeout * 1000;
      if (setsockopt(pmaps[i].sock, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeo,
                     sizeof(timeo))) {
        winet_log(WINET_LOG_ERROR,
                  "[%s] unable to set socket option: opt=SO_SNDTIMEO\n",
                  WINET_APPNAME);
        return -1;
      }
    }
    ...
    if (setsockopt(pmaps[i].sock, SOL_SOCKET, SO_LINGER, (char *)&ling,
                   sizeof(ling))) {
      winet_log(WINET_LOG_ERROR,
                "[%s] unable to set socket option: opt=SO_LINGER\n",
                WINET_APPNAME);
      return -1;
    }

    nodelay = 1;
    if (setsockopt(pmaps[i].sock, IPPROTO_TCP, TCP_NODELAY, (char *)&nodelay,
                   sizeof(nodelay))) {
      winet_log(WINET_LOG_ERROR,
                "[%s] unable to set socket option: opt=TCP_NODELAY\n",
                WINET_APPNAME);
      return -1;
    }
    ...
    if (pmaps[i].port != 44344) {
      winet_log(WINET_LOG_ERROR, "Only port 44344 is allowed in STIG mode\n",
                WINET_APPNAME);
      return -1;
    }
#endif
...
static void winet_cleanup(void) {
  ...
  /* TODO: Looks like we don't wait for threads, that may use 'user','pass' or
   * 'envSnapshot'  */
...
static int winet_user_handle(portmap_t *pm, HANDLE *husr) {
  ...
  if (!LogonUserA(pm->user, ".", pm->pass, LOGON32_LOGON_BATCH,
                  LOGON32_PROVIDER_DEFAULT, &hlog)) {
    winet_log(WINET_LOG_ERROR,
              "[%s] unable to logon user: user='%s' pass='%s' err='%s'\n",
...
static int winet_create_stdhandles(HANDLE *in, HANDLE *out, HANDLE *err,
  ...
  if (!DuplicateHandle(GetCurrentProcess(), p2s_their, GetCurrentProcess(), err,
                       0, TRUE, DUPLICATE_SAME_ACCESS)) {
    pWin32Error("DuplicateHandle() failed");
    goto err3;
  }

  if (!DuplicateHandle(GetCurrentProcess(), p2s_their, GetCurrentProcess(), out,
                       0, TRUE, DUPLICATE_SAME_ACCESS)) {
    pWin32Error("DuplicateHandle() failed");
    goto err4;
  }
...
static LPVOID winet_prepare_env(portmap_t *pm, SOCKET asock,
  ...
  newsize = envSnapshotNBytes + sizeof(CLIENT_IP _T("=") LONGEST_ADDR
                                                 _T("\0") CLIENT_PORT
                                                 _T("=") LONGEST_PORT _T("\0"));
...
static int winet_serve_client(thread_data_t *thd) {
  ...
  if (winet_create_stdhandles(&si.hStdInput, &si.hStdOutput, &si.hStdError,
                              &hPipeOurWrite, &thd->hPipeOurRead) < 0)
    return -1;
  ...
  if (!pm->pass) {
    if (!CreateProcessA(NULL, pm->cmdline, NULL, NULL, TRUE, WINET_CHILD_FLAGS,
                        env, NULL, &si, &pi)) {
      winet_log(WINET_LOG_ERROR,
                "[%s] unable to create process: cmdln='%s' err='%s'\n",
    ...
    if (!CreateProcessAsUserA(husr, NULL, pm->cmdline, NULL, NULL, TRUE,
                              WINET_CHILD_FLAGS, env, NULL, &si, &pi)) {
      winet_log(WINET_LOG_ERROR,
                "[%s] unable to create process as user: cmdln='%s' user='%s' "
                "err='%s'\n",
                WINET_APPNAME, pm->cmdline, pm->user,
...
static int winet_handle_client(portmap_t *pm, SOCKET asock,
  ...
  if (!(hthr = (HANDLE)_beginthreadex(NULL, 0, winet_thread_proc, thd, 0,
                                      &thrid))) {
    free(thd);
    return -1;
  }
...
int winet_main(int argc, char const **argv) {
  ...
  if (winet_load_cfg(cfgfile) < 0 || winet_create_listeners() < 0) {
    WSACleanup();
    return 2;
  }
      ...
      if ((asock = accept(pmaps[i].sock, (struct sockaddr *)&saddr, &adrlen)) !=
        ...
        if (winet_handle_client(&pmaps[i], asock, &saddr) < 0) {
          winet_log(WINET_LOG_ERROR, "[%s] unable to serve client: %s:%d\n",
                    WINET_APPNAME, inet_ntoa(saddr.sin_addr),
                    (int)ntohs(saddr.sin_port));

          closesocket(asock);
        } else {
          winet_log(WINET_LOG_MESSAGE,
                    "[%s] %s: client served: %s:%d -> '%s'\n", WINET_APPNAME,
                    timestamp(), inet_ntoa(saddr.sin_addr),
                    (int)ntohs(saddr.sin_port), pmaps[i].cmdline);
        }
      }
    }
  }
